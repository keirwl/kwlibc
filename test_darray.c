...
MU_TEST(slice) {
    ...
    DArray one_slice;
    retval = darray_slice(&one_slice, &source, 0, 1);
    mu_check(retval == 0);
    ...
    DArray full_slice;
    retval = darray_slice(&full_slice, &source, 0, 5);
    mu_check(retval == 0);
    mu_check(darray_len(&full_slice) == 5);
    for (int i = 0; i < 5; ++i) {
...
MU_TEST(slice_indices) {
    ...
    DArray slice_with_neg;
    retval = darray_slice(&slice_with_neg, &source, 1, -1);
    mu_check(retval == 0);
    ...
    // Slice with out-of-bound indices - should fail
    DArray slice_with_oob_indices;
    retval = darray_slice(&slice_with_oob_indices, &source, 0, 8);
    mu_check(retval == -2);

    // Slice with start later than end - should fail
    DArray slice_with_reversed_indices;
    retval = darray_slice(&slice_with_reversed_indices, &source, 4, 1);
    mu_check(retval == -3);

    // Slice that tries to "wrap around" from negative index to start - should fail
    DArray *slice_with_wrap_around;
    retval = darray_slice(slice_with_wrap_around , &source, -1, 0);
    mu_check(retval == -3);
